...
/* 存放get请求的相关数据 */
static uint8_t https_get_buffer[1342];
/* 存放接收到GET请求的数据 */
...
static esp_err_t http_url_parse(char *URL, char *host, char *filename)
{
  ...
  os_memset(host, 0, strlen(host)/sizeof(char));
  os_memset(filename, 0, strlen(filename)/sizeof(char));
  PA = URL;
...
static void https_get_reuest_json_data_parse(cJSON *json_data)
{
  ...
  char * city=cJSON_Print(cJSON_GetObjectItem(location, "name"));
  char * daily0=cJSON_Print(cJSON_GetArrayItem(daily, 0));
  char * daily1=cJSON_Print(cJSON_GetArrayItem(daily, 1));
  char * daily2=cJSON_Print(cJSON_GetArrayItem(daily, 2));
  ESP_LOGI(TAG,"CITY:%s\n%s\n%s\n%s\n",city,daily0,daily1,daily2);
  free(city);
  free(daily0);
  free(daily1);
  free(daily2);
...
//  https_get_reuest_json_data_parse(cJSON_Parse(os_strchr(json_data, '{')));  
  cjson_to_struct_info(json_data);
  os_free(json_data);
  vTaskDelete(NULL);
}
...
{
  int ret, len;  
  recv_json_buff = (char*)os_malloc(sizeof(char)*2047);
...
//    ESP_LOGI(TAG,"size of buffer is %d\n",len);
    memset(recv_buff, 0, sizeof(recv_buff)-1);
//    ESP_LOGI(TAG,"set buffer zero\n");
...
int https_request_by_GET(char *URL)
{
  ...
  {
  ESP_LOGI("https_request_by_GET", "URL parse failed");
